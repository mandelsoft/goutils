...
var _ = Describe("ChunkedReader", func() {
	...
	var chunked *ioutils.ChunkedReader
	...
	Context("max preread", func() {
		BeforeEach(func() {
			chunked = ioutils.NewChunkedReader(buf, 5)
		})
	...
	Context("complete", func() {
		BeforeEach(func() {
			chunked = ioutils.NewChunkedReader(buf, 100)
		})
	...
	Context("non-matching chunk size", func() {
		BeforeEach(func() {
			chunked = ioutils.NewChunkedReader(buf, 15)
		})
	...
	Context("chunk size matches read size", func() {
		BeforeEach(func() {
			chunked = ioutils.NewChunkedReader(buf, 20)
		})
...
func check(chunked *ioutils.ChunkedReader, n int, buf []byte, exp int, data string, done, next bool) {
