...
func getGenericData(in datautils.GenericData) (datautils.GenericData, error) {
	data, err := datautils.GetGenericData(in)
	if err != nil {
		if !errors.IsErrInvalidKind(err, datautils.KIND_DATASOURCE) {
			return nil, err
		}
		return in, nil
	} else {
