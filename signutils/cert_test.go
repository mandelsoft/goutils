...
var _ = Describe("normalization", func() {
	...
	capriv, capub, err := rsa.CreateKeyPair()
	...
	priv, pub, err := rsa.CreateKeyPair()
	Expect(err).To(Succeed())
	...
	Context("foreignly signed", func() {
		...
		It("verifies for issuer", func() {
			err = signutils.VerifyCert(nil, pool, "mandelsoft", cert)
			Expect(err).To(Succeed())
		})
		It("verifies for anonymous", func() {
			err = signutils.VerifyCert(nil, pool, "", cert)
			Expect(err).To(Succeed())
		})
		It("fails for wrong issuer", func() {
			err = signutils.VerifyCert(nil, pool, "x", cert)
